...
### Setting up the Spotify API end:
- Create an app on the [Spotify dashboard](https://developer.spotify.com/dashboard)
- Inside the app, note down your Client ID and Client secret. Set the Redirect URI field to `http://127.0.0.1:8888/callback`.
...
- Once the app is done being created, create a folder named 'data' and create a `client.txt` file. Inside, put the following on the first 4 lines:
  -  Client ID
  -  Client Secret
  -  Redirect URI (should be `http://127.0.0.1:8888/callback`)
  -  Username
...
- If everything looks good, go to your `.zshrc` (or `.bashrc` if using bash) and add the following snippet of code to the bottom of the file. Make sure to replace the `[PATH]` with the directory you set earlier.
...
- Add `spotifetch` to the end of your `.zshrc` (or `.bashrc if using bash). If you already had `fastfetch` in there, comment it out or remove it entirely.
- Add the `main.py` file as a systemd user service so that it runs on boot (Google a tutorial if needed). This is to constantly check for what song is being streamed.
- Reboot the system.
